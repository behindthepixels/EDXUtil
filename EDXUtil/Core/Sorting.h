...
namespace EDX
{
	...
	*/
	...
	struct Less
	{
		__forceinline bool operator()(const T& A, const T& B) const { return A < B; }
	};
	...
	*/
	...
	struct Greater
	{
		__forceinline bool operator()(const T& A, const T& B) const { return B < A; }
	};
	...
	*/
	...
	struct DereferenceWrapper
	{
		...
		DereferenceWrapper(const PREDICATE_CLASS& InPredicate)
			: Predicate(InPredicate) {}

		/** Pass through for non-pointer types */
		__forceinline bool operator()(T& A, T& B) { return Predicate(A, B); }
		__forceinline bool operator()(const T& A, const T& B) const { return Predicate(A, B); }
	};
	/** Partially specialized version of the above class */
	template<typename T, class PREDICATE_CLASS>
	struct DereferenceWrapper<T*, PREDICATE_CLASS>
	{
		const PREDICATE_CLASS& Predicate;
	...
	*/
	...
	void SortInternal(T* First, const int32 Num, const PREDICATE_CLASS& Predicate)
	{
		...
		Stack RecursionStack[32] = { { First,First + Num - 1 } }, Current, Inner;
		for (Stack* StackTop = RecursionStack; StackTop >= RecursionStack; --StackTop) //-V625
		{
			Current = *StackTop;
		Loop:
	...
	*/
	...
	{
		SortInternal(First, Num, DereferenceWrapper<T, PREDICATE_CLASS>(Predicate));
	}
	...
	*/
	...
	{
		SortInternal(First, Num, DereferenceWrapper<T, Less<T> >(Less<T>()));
	}
	...
	public:
		...
		*/
		...
		static void Rotate(T* First, const int32 From, const int32 To, const int32 Amount)
		{
			...
			{
				T BufferObject = Move(First[From + Index]);
	...
	*/
	...
	{
		MergeSort<RotationInPlaceMerge<JugglingRotation<EuclidDivisionGCD> > >::Sort(First, Num, Predicate);
	}
	...
	*/
	...
	{
		StableSortInternal(First, Num, DereferenceWrapper<T, PREDICATE_CLASS>(Predicate));
	}
	...
	*/
	...
	{
		StableSortInternal(First, Num, DereferenceWrapper<T, Less<T> >(Less<T>()));
	}
